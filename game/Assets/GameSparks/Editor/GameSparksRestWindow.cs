...
namespace GameSparks.Editor
{
    ...
    public class GameSparksRestWindow : EditorWindow
    {
        ...
        void OnGUI()
        {
            ...
            {
                result = GameSparksRestApi.getDownloadable(GameSparksSettings.ApiKeyFinal(), userName, password, shortCode);
            }

            if (GUILayout.Button("POST"))
            {
                result = GameSparksRestApi.setDownloadable(GameSparksSettings.ApiKeyFinal(), userName, password, shortCode, fileName);
            }
