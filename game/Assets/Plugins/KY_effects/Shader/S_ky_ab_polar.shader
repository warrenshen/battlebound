...
////// Lighting:
////// Emissive:
                float2 node_5869 = (i.uv0*_polarDensity);
                float node_5942 = 0.0;
                float node_5886 = (_polarDensity*(-1.0));
                float2 node_2818 = (node_5886 + ( (node_5869 - node_5942) * (_polarDensity - node_5886) ) / (_polarDensity - node_5942));
                float node_1092 = length(node_2818);
                float2 node_8219 = node_2818.rg;
                float2 node_4114 = float2(node_1092,((atan2(node_8219.g,node_8219.r)/6.28318530718)+0.5));
                float2 node_3714 = (node_4114+float2(_polarOffsetX,_polarOffsetY));
                float4 _baseTex_var = tex2D(_baseTex,TRANSFORM_TEX(node_3714, _baseTex));
                float3 node_8758 = pow(_baseTex_var.rgb,_baseTexDensity);
                float3 emissive = (node_8758*_emissive*i.vertexColor.rgb);
                float3 finalColor = emissive;
                float4 _maskTex_var = tex2D(_maskTex,TRANSFORM_TEX(i.uv0, _maskTex));
                return fixed4(finalColor,((pow(_maskTex_var.r,_alphaDensity)*_alphaPower)*i.vertexColor.a*saturate(pow((i.vertexColor.r+i.vertexColor.g+i.vertexColor.b),_vertColorDensity))));
            }
            ENDCG
        }
    }
