Shader "HyperCard/Card (Face)"
{
	Properties
	{
		...
		[HideInInspector] _Holo_Enabled("_Holo_Enabled", Int) = 0
		[HideInInspector] _Holo_Debug("_Holo_Debug", Int) = 0
		[HideInInspector] _HoloMask("_HoloMask", 2D) = "black" {}
		[HideInInspector] _HoloMap("_HoloMap", 2D) = "black" {}
		[HideInInspector] _HoloMap_Scale("_HoloMap_Scale", Vector) = (1,1,0,0)
		[HideInInspector] _HoloCube("_HoloCube", Cube) = "_Skybox" {}
		[HideInInspector] _HoloCubeColor("_HoloCubeColor", Color) = (0,0,0,0)
		[HideInInspector] _HoloCubeRotation("_HoloCubeRotation", Float) = 0
		[HideInInspector] _HoloPower("_HoloPower", Float) = 1
		[HideInInspector] _HoloAlpha("_HoloAlpha", Float) = 1
		[HideInInspector] _HoloBBoxMin("_HoloBBoxMin", Vector) = (1,1,1,0)
		[HideInInspector] _HoloBBoxMax("_HoloBBoxMax", Vector) = (1,1,1,0)
		[HideInInspector] _HoloEnviCubeMapPos("_HoloEnviCubeMapPos", Vector) = (0,0,0,0)
	...
	SubShader
	{
		...
		Pass
		{
			...
			uniform sampler2D _CardPicture;
			uniform float4 _CardPicture_ST;
			...
			float4 frag(v2f IN) : SV_TARGET
			{
				...
				/* DISTORTION */
				/* ---------------------------------------------------------------------------------------*/
				float2 uv = TRANSFORM_TEX(IN.texcoord, _CardPicture);
				...
				if (_Dist0_Enabled == 1) 
				{
					...
					cardpicture_col = tex2D(_CardPicture, float2(uv.x + dist0_red_c + dist0_green_c + dist0_blue_c,
						uv.y + dist0_red_c + dist0_green_c + dist0_blue_c));
				}
				else 
				{
					cardpicture_col = tex2D(_CardPicture, float2(uv.x, uv.y));
				}
				...
				if (_Holo_Enabled == 1) 
				{
					float3 viewDirWS = normalize(IN.viewDirInWorld);
					float3 normalWS = normalize(IN.normalInWorld);

					float3 reflDirWS = reflect(viewDirWS, normalWS);

					float4 holo_mask = tex2D(_HoloMask, uv);
					holo_mask.r *= _HoloAlpha;

					float3 localCorrReflDirWS = LocalCorrect(reflDirWS, _HoloBBoxMin, _HoloBBoxMax, IN.vertexInWorld, _HoloEnviCubeMapPos);

					float4 holo_map = tex2D(_HoloMap, uv * _HoloMap_Scale);
					holo_map *= _HoloPower;
					float2 holo_o = holo_map.rg + float4(localCorrReflDirWS, 0).rg;
					...
					if(_Holo_Debug) 
					{
						cardpicture_col = reflColor;
					}
					else 
					{
						half3 hcol = cardpicture_col.rgb * (1 - holo_mask.r);
						half3 hrefl = reflColor.rgb * holo_mask.r;

						cardpicture_col = float4(hcol + hrefl, 1);
					}	
				}
