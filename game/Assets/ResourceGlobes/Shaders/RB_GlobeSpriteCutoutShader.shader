Shader "Custom/GlobeSpriteCutoutShaderV2" {

	Properties{
		_Color("Color", Color) = (1,1,1,1)
		_HotlineColor("Hotline Color", Color) = (1,1,1,1)
		_MainTex("Sprite", 2D) = "white" {}
		_MaskTex("Albedo 1 (RGB)", 2D) = "white" {}
...
#pragma surface surf NoLighting alpha:blend
	...
	void surf(Input IN, inout SurfaceOutput o) {
		...
		float2 uv1 = float2(IN.uv_MainTex.x, IN.uv_MainTex.y);
		float2 uvmasx = float2(IN.uv_MaskTex.x + _Time[1] * _LAYER_1_SCROLL_X, IN.uv_MaskTex.y + _Time[1] * _LAYER_1_SCROLL_Y);
		...
		fixed4 c = tex2D(_MaskTex, uvmasx) * tex2D(_MainTex2, uv2) * tex2D(_MainTex2, uv2 + 0.3)  * tex2D(_MainTex3, uv3) * _Color;
		fixed4 a = tex2D(_MainTex, uv1);
		fixed4 x = tex2D(_HotlineTex, uvx) * _HotlineColor;
		fixed4 t = tex2D(_CutoutTex, IN.uv_CutoutTex);
