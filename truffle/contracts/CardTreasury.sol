...
contract CardMint is CardBase {
  ...
  /* EVENTS */
  event TemplateMinted(uint256 templateId);
  event CardMinted(address owner, uint256 cardId);
  ...
  function mintCard(uint256 _templateId, uint256 _variation, address _owner) external onlyOwner returns (uint256) {
    require(_owner != address(0));
    return _mintCard(_templateId, _variation, _owner);
  }

  function mintCards(uint256[] _templateIds, uint256[] _variations, address _owner) external onlyOwner {
    require(_owner != address(0));
