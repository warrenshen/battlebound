...
describe("challenge play structure", function() {
  ...
  describe("play creature in front of structure", function() {
    const challengeStateData = {
      "current": {
        "ID_OPPONENT": {
          "hasTurn": 0,
    ...
    it("should support play creature grant taunt", function() {
      return new Promise((resolve) => {
        gamesparks.sendWithData(
          "LogEventRequest",
          {
          ...
          function(response) {
            ...
            assert.equal(lastMoves[0].attributes.cardId, "ID_PLAYER-16");
            ...
            assert.equal(playerField[0].abilities.length, 3);
            assert.equal(playerField[0].abilities.indexOf(1) >= 0, true);
